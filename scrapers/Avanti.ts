...
export class Avanti implements Scraper {
  ...
  async getOpportunityFromPage(url: string): Promise<Opportunity> {
    ...
    const title = main.children("header").children("div").children("h1").text()
      .trim();

    let subtitle = "";

    main.children("header").children("div").children(".text-container")
      .children("p").each((_, child) => {
    ...
    main.children(".frame-content").children(".text-container").each(
      (_, section) => {
        $(section).children().each((_, child) => {
          ...
          if (child.tagName == "h2") {
            ...
            $(child).children().each((index, mini_child) => {
              ...
              if (mini_child.tagName == "h4") {
                description += "\n" +
                  $(mini_child).text().trim().toUpperCase() + "\n";
              } else if (mini_child.tagName == "h5") {
                description += $(mini_child).text().trim().toUpperCase() + "\n";
              } else if (mini_child.tagName == "p") {
                description += $(mini_child).text().trim();
